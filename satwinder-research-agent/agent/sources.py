"""
Data source fetchers. All use free public APIs — no keys required.
"""
...
def fetch_pubmed(query: str, since: date, max_results: int = 15) -> list:
    """Search PubMed for recent papers matching `query` since `since`."""
    ...
    # 2) esummary for metadata (title, journal, date)
    ...
    try:
        ...
        if r.ok:
            ...
            for article in root.iter('PubmedArticle'):
                ...
                # Join multiple AbstractText elements
...
def fetch_clinical_trials(since: date, max_results: int = 30) -> list:
    """Recently updated prostate cancer trials from ClinicalTrials.gov v2 API."""
    params = {
...
def fetch_fda(since: date, max_results: int = 15) -> list:
    """FDA press releases filtered for oncology/prostate keywords."""
