...
Runs via GitHub Actions every morning. Pulls from:
  - PubMed (E-utilities API)
  - ClinicalTrials.gov (API v2)
  - FDA press releases (RSS)

Uses Claude to summarize and tag each item, then writes results to
data/latest.json (which the dashboard reads).
"""
...
# Search terms for PubMed — kept focused on advanced/metastatic disease
PUBMED_QUERIES = [
...
# Max items to send to Claude in one batch (controls API cost)
...
def main():
    ...
    # Sort by date (most recent first) and cap
